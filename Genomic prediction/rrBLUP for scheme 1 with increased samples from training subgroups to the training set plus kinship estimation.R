    ...
    for (p in 1:length(pheno_all)) {
      ...
      for (j in 1:length(percentage_train)) {
        pheno_train_all <- rbind(pheno_all[!rownames(pheno_all) %in% rownames(pheno_group[[i]]), ][train,],
                                 pheno_all[!rownames(pheno_all) %in% rownames(pheno_group[[i]]), ][percentage_train[[j]], ])
        
        geno_train_all <- rbind(geno_all[!rownames(geno_all) %in% colnames(geno_group[[i]]), ][train,],
                                geno_all[!rownames(geno_all) %in% colnames(geno_group[[i]]), ][percentage_train[[j]], ])
        ...
        names(file_percen)[j] <- names(percentage_train)[j]
        pheno_train_file[[j]] <- pheno_train_all
        names(pheno_train_file)[j] <- names(percentage_train)[j]
        geno_train_file[[j]] <- geno_train_all
        names(geno_train_file)[j] <- names(percentage_train)[j]
